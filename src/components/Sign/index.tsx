  ...
  const [isPairingCodeValid, setIsPairingCodeValid] = useState(false);
    ...
    const onStart = async function () {
        try {
            ...
            ipcRenderer.on('onSessionRequest', (event, message) => {
                console.log("onSessionRequest message: ",message);
                if(!message.method) throw Error("Invalid onSessionRequest method")
                if(!message.requestEvent) throw Error("Invalid onSessionRequest requestEvent")
                if(!message.requestSession) throw Error("Invalid onSessionRequest requestSession")
                setRequestEvent(message.requestEvent);
                setRequestSession(message.requestSession);
                openModal('onSessionRequest');
            });
                ...
                <ModalContent >
                    ...
                    <ModalBody>
                        ...
                        {modalType === 'onSessionRequest' && (
                            <div>
                                onSessionRequest
                                <Sign requestEvent={requestEvent} requestSession={requestSession} onClose={onClose}/>
                            </div>
                        )}
                    </ModalBody>
                    <ModalFooter>
